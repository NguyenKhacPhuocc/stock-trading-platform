...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!upstream.ok) {
      const em = pickUpstreamErrorMessage(body, 'Không tải được danh sách lệnh');
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const body = toBodyOrNull(text);
    if (!upstream.ok) {
      const em = pickUpstreamErrorMessage(body, 'Đặt lệnh thất bại');
      return gwResError(em, { httpStatus: upstream.status, ec: upstream.status });
    }
    ...
    return gatewayUpstreamCatch(e, {
      timeout: 'Hết thời gian chờ đặt lệnh',
      connect: 'Lỗi kết nối máy chủ lệnh',
    });
  } finally {
    clearTimeout(timer);
