...
function backendQuery(req: NextRequest): string {
  const keys = ['from', 'to', 'limit', 'offset'] as const;
...
export async function GET(req: NextRequest) {
  ...
  try {
    const upstream = await fetchBackendUpstream({
      req,
      path: `/api/wallet/sell-fills${backendQuery(req)}`,
      tradingAccountId,
      signal: ac.signal,
      errorFallback: 'Không tải được lịch sử bán',
    });
    if (!upstream.ok) {
    ...
    return gatewayUpstreamCatch(e, {
      timeout: 'Hết thời gian chờ dữ liệu',
